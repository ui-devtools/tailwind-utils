// TODO: bring in scale properties and refactor format for that

// Fork of https://github.com/tailwindlabs/tailwindcss/blob/master/src/corePlugins.js
...
const skipped = ['boxShadow', 'ringWidth', 'ringOpacity', 'animation'];
const createUtilityPlugin = (themeKey, utilityVariations = [[themeKey, [themeKey]]], options = {}) => {
  // TODO
  if (utilityVariations[1]) {
    skipped.push(themeKey);
    return;
  }

  const [prefix, properties] = utilityVariations[0];
  ...
  scaleProperties[themeKey] = { prefix, scale: themeKey };
  if (options.supportsNegativeValues) scaleProperties[themeKey].supportsNegativeValues = true;
...
let corePlugins = {
  ...
  translate: createUtilityPlugin(
    ...
    [
      [
        ['translate-x', [['@defaults transform', {}], '--tw-translate-x', ['transform', cssTransformValue]]],
        ['translate-y', [['@defaults transform', {}], '--tw-translate-y', ['transform', cssTransformValue]]]
      ]
    ],
    { supportsNegativeValues: true }
  ),
  ...
  scale: createUtilityPlugin(
    'scale',
    [
      ['scale', [['@defaults transform', {}], '--tw-scale-x', '--tw-scale-y', ['transform', cssTransformValue]]],
      [
      ...
      matchUtilities(
        {
          via: (value) => {
            ...
            return {
              '--tw-gradient-to': transparentToValue,
              '--tw-gradient-stops': `var(--tw-gradient-from), ${toColorValue(value, 'via')}, var(--tw-gradient-to)`
            };
          }
        },
        options
  ...
  fontFamily: ({ matchUtilities, theme }) => {
    matchUtilities(
      {
        font: (value) => {
          let [families, options = {}] = Array.isArray(value) && isPlainObject(value[1]) ? value : [value];
          let { fontFeatureSettings } = options;

          return {
            'font-family': Array.isArray(families) ? families.join(', ') : families,
            ...(fontFeatureSettings === undefined ? {} : { 'font-feature-settings': fontFeatureSettings })
          };
        }
      },
      {
...
let namedClassProperties = {};

const addUtilities = (namedUtilities) => {
  Object.keys(namedUtilities).forEach((className) => {
    const styles = namedUtilities[className];
    namedClassProperties[className] = styles;
  });
};
const config = () => {};
...
Object.keys(corePlugins).forEach((key) => {
  if (typeof corePlugins[key] === 'function') {
    const fn = corePlugins[key];
    if (fn.toString().includes('matchUtilities') || fn.toString().includes('addDefaults')) {
      skipped.push(key);
    } else fn({ addUtilities, config });
  }
});

// themeKey: 'zIndex'
// utilityVariations: [ ['z', ['zIndex']] ]
// options: { supportsNegativeValues: true })
